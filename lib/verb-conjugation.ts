...
export const VERB_CONJUGATION_SUPPORTED_SOURCE_LANGUAGES = ["de", "fr"] as const;
...
export function getConjugationMoodLabelKeys(language: SupportedConjugationLanguage): Record<string, I18nKey> {
  return language === "de"
    ? (GERMAN_CONJUGATION_MOOD_LABEL_KEYS as Record<string, I18nKey>)
    : (FRENCH_CONJUGATION_MOOD_LABEL_KEYS as Record<string, I18nKey>);
}

export function getConjugationTenseLabelKeys(language: SupportedConjugationLanguage): Record<string, I18nKey> {
  return language === "de"
    ? (GERMAN_CONJUGATION_TENSE_LABEL_KEYS as Record<string, I18nKey>)
    : (FRENCH_CONJUGATION_TENSE_LABEL_KEYS as Record<string, I18nKey>);
}
